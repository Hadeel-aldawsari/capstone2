...
public class ReviewController {
    ...
    public ResponseEntity postItemReview(@RequestBody @Valid Review review, Errors errors){
        if(errors.hasErrors()){
    ...
    public ResponseEntity posPackageReview(@RequestBody @Valid Review review, Errors errors){
        if(errors.hasErrors()){
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
